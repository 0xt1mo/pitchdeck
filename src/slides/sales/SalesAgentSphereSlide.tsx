...
const enterprises = [
  ...
  { name: 'REGULATOR', color: '#06b6d4', angle: 198, dist: 210, agents: 3 },
];
...
function NetworkDiagram() {
  ...
  return (
    <svg viewBox="0 0 600 600" className="w-full h-auto">
      ...
      <text x={CX} y={CY + 7} textAnchor="middle" fill="#fb923c" fontSize="20" fontWeight="bold"
        fontFamily="Anton, sans-serif" letterSpacing="0.12em">MARKETPLACE</text>
      ...
      {enterprises.map((ent, i) => {
        ...
        return (
          <g key={ent.name}>
            ...
            <text x={ex} y={ey + 58} textAnchor="middle" fill={ent.color} fontSize="19"
              fontWeight="bold" fontFamily="Anton, sans-serif" letterSpacing="0.1em">
              {ent.name}
            </text>
            ...
            <text x={gx} y={gy + 30} textAnchor="middle" fill="#c4a3f8" fontSize="11"
              fontFamily="Geist Mono, monospace" letterSpacing="0.08em" fontWeight="bold">
              SIF GATEWAY
            </text>
          </g>
        );
...
const features = [
  {
    title: 'CROSS-ENTERPRISE DISCOVERY',
    text: 'Agents in your organisation locate counterparties in others — capabilities, identity, and reputation, signed and validated by their respective SIF gateways.',
  },
  {
    title: 'GATEWAY-MEDIATED MESSAGING',
    text: 'Encrypted agent-to-agent communication between enterprises. Every message authenticated, every interaction logged at the gateway — never directly agent-to-agent.',
  },
  {
    title: 'INTENT-BASED COORDINATION',
    text: 'Agents express what they need — purchase, settlement, data exchange. SIF gateways enforce the originating enterprise\'s policy on every cross-boundary interaction.',
  },
  {
    title: 'PROTOCOL NEUTRAL',
    text: 'MCP, A2A, x402, AP2 — your agents reach the protocol the counterparty already speaks. No single-protocol bet, no rip-and-replace.',
  },
];
...
export function SalesAgentSphereSlide() {
  return (
    <SlideFrame
      title="WHAT THIS ENABLES NEXT:"
      accent="CROSS-PERIMETER COORDINATION."
      subtitle="Once Unicity is deployed inside your perimeter, your agents can transact with agents at partner enterprises, regulators, and government counterparties — through SIF gateways, never directly. Zero trust by default."
    >
      <div className="grid grid-cols-[1fr_1.05fr] gap-8 lg:gap-12 items-center">
        <motion.div
