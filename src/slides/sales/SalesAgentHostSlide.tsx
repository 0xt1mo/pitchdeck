...
const T = tr({
  en: {
    diagUserSpace: 'USER SPACE',
    diagUserSpaceSub: 'User-facing tools & agent frameworks',
    diagUserBoxes: ['Claude Code', 'OpenClaw', 'Custom Agent Tools', 'LLM Frontends'],
    diagAos9: 'AOS-9 RUNTIME',
    diagAos9Sub: 'User-space microkernel — enforcement layer',
    diagAos9Row1: ['Security Interceptor', 'WASM Sandbox', 'Capsule Engine'],
    diagAos9Row2: ['Audit & Budgets', 'IPC Message Bus'],
    diagHostOs: 'HOST OS',
    diagHostSub: 'Underlying operating system',
    diagHostBoxes: ['Linux', 'macOS', 'Container Runtimes'],
    headBrand: 'AOS-9',
    headRest: '— THE SECURE AGENTIC RUNTIME',
    subHead: (
      <>
  ...
  pt: {
    diagUserSpace: 'ESPAÇO DE USUÁRIO',
    diagUserSpaceSub: 'Ferramentas e frameworks de agente para o usuário',
    diagUserBoxes: ['Claude Code', 'OpenClaw', 'Ferramentas de Agente Personalizadas', 'Frontends de LLM'],
    diagAos9: 'AOS-9 RUNTIME',
    diagAos9Sub: 'Microkernel em espaço de usuário — camada de imposição',
    diagAos9Row1: ['Interceptador de Segurança', 'Sandbox WASM', 'Motor de Cápsulas'],
    diagAos9Row2: ['Auditoria e Orçamentos', 'Barramento de Mensagens IPC'],
    diagHostOs: 'HOST OS',
    diagHostSub: 'Sistema operacional subjacente',
    diagHostBoxes: ['Linux', 'macOS', 'Runtimes de Contêiner'],
    headBrand: 'AOS-9',
    headRest: '— O RUNTIME AGÊNTICO SEGURO',
    subHead: (
      <>
        O AOS-9 é o OS de imposição segura sobre o qual ferramentas como Claude Code e OpenClaw <span className="text-orange-400">rodam por cima</span>, não ao lado.
        Um microkernel em espaço de usuário com syscalls, airlocks e isolamento em sandbox — o kernel permanece mínimo enquanto todo o resto vive em espaço de usuário.
      </>
    ),
    keyPoints: [
      { title: 'Microkernel em Espaço de Usuário', text: 'O AOS-9 se modela como um OS — com syscalls, airlocks e isolamento em espaço de usuário. O kernel (aos9d) e o SDK do sistema (aos9-sys) são os únicos componentes centrais.' },
      { title: 'Sandbox de Cápsulas WASM', text: 'Todos os plugins rodam como cápsulas WASM dentro de um runtime em sandbox. Plugins OpenClaw são compilados em cápsulas via aos9-openclaw, com chamadas de host encaminhadas para aos9::sys.' },
      { title: 'Camada de Imposição', text: 'Interceptação de segurança, imposição de orçamento e registro de auditoria acontecem no nível do runtime — abaixo do agente, não ao lado dele. As ferramentas não conseguem contornar o kernel.' },
      { title: 'Tudo É Espaço de Usuário', text: 'CLIs, OpenClaw, frontends e até provedores de LLM são componentes externos em espaço de usuário que se comunicam por IPC. O kernel permanece mínimo e alheio a ABIs legadas.' },
    ],
  },
});
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      <text x={px + 16} y={y0 + 24} fill="#3b82f6" fontSize="14" fontWeight="bold"
        fontFamily={fontTitle} letterSpacing="0.08em">{T.diagUserSpace}</text>
      <text x={px + 145} y={y0 + 24} fill="rgba(254,254,254,0.3)" fontSize="9"
        fontFamily={font}>{T.diagUserSpaceSub}</text>
      {T.diagUserBoxes.map((t, i) => {
      ...
      <text x={px + 16} y={y1 + 24} fill="#f97316" fontSize="15" fontWeight="bold"
        fontFamily={fontTitle} letterSpacing="0.08em">{T.diagAos9}</text>
      <text x={px + 180} y={y1 + 24} fill="rgba(254,254,254,0.35)" fontSize="9"
        fontFamily={font}>{T.diagAos9Sub}</text>
      {T.diagAos9Row1.map((t, i) => {
      ...
      <text x={px + 16} y={y2 + 24} fill="#6366f1" fontSize="14" fontWeight="bold"
        fontFamily={fontTitle} letterSpacing="0.08em">{T.diagHostOs}</text>
      <text x={px + 105} y={y2 + 24} fill="rgba(254,254,254,0.3)" fontSize="9"
        fontFamily={font}>{T.diagHostSub}</text>
      {T.diagHostBoxes.map((t, i) => {
        const tw = i === 2 ? 130 : 80;
      ...
      <div className="relative z-10 h-full flex flex-col px-6 sm:px-10 lg:px-16 py-10 sm:py-14 justify-center gap-6">
        ...
        <div className="shrink-0">
          <motion.h1 initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }}
            ...
            <span className="text-orange-400">{T.headBrand}</span>{' '}
            {T.headRest}
          </motion.h1>
