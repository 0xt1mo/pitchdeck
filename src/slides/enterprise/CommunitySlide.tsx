...
// Non-text data: X-axis tick coordinates; month labels come from T.xMonths.
const X_TICKS = [0, 50, 100, 150, 200, 250, 300, 350];

const T = tr({
  en: {
    eyebrow: 'Appendix — Traction',
    headLead: 'BUILDING THE',
    headAccent: 'UNICITY COMMUNITY',
    peakHashRate: 'Peak hash rate',
    explanation: (
      <>
    ...
    chartTitle: 'Difficulty (log scale)',
    chartCaption: 'Jun–Dec 2025',
    xMonths: ['Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', ''],
  },
  pt: {
    eyebrow: 'Apêndice — Tração',
    headLead: 'CONSTRUINDO A',
    headAccent: 'COMUNIDADE UNICITY',
    peakHashRate: 'Taxa de hash de pico',
    explanation: (
      <>
        A testnet alpha rodou de junho a dezembro de 2025. Zero gasto com marketing, sem redes sociais, apenas boca a boca — a comunidade cresceu organicamente, atingindo o pico de <span className="text-orange-400">1GH/s</span>, atrás apenas do Monero com 5GH/s entre as chains RandomX.
      </>
    ),
    chartTitle: 'Dificuldade (escala log)',
    chartCaption: 'Jun–Dez 2025',
    xMonths: ['Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez', ''],
  },
});
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      <text x={pad.left + 4} y={pad.top - 10} fill="rgba(254,254,254,0.6)" fontSize="11"
        fontFamily="Geist Mono, monospace">{T.chartTitle}</text>
          ...
          <g key={val}>
            ...
            {label && (
              <text x={x} y={H - 8} textAnchor="middle" fill="rgba(249,115,22,0.4)"
                fontSize="8" fontFamily="Geist Mono, monospace">{label}</text>
            )}
          </g>
        );
      })}
