...
// Non-text data: step numbers + lifeline indices (from/to/at) stay outside;
// the display labels live in T, indexed parallel to these arrays.
type ArrowStep = { num: number; from: number; to: number };
type SelfStep = { num: number; at: number };
...
const T = tr({
  en: {
    eyebrow: 'Appendix — Protocol',
    headLead: 'UNICITY X402 —',
    headAccent: 'THE MESSAGE IS THE PAYMENT',
    subHead: 'Native HTTP payments with instant finality, zero gas fees, and no wallet complexity',
    tradTitle: 'Traditional Blockchain X402',
    tradBadge: '12 steps',
    uniTitle: 'Unicity Frictionless X402',
    uniBadge: '5 steps',
    tradParticipants: ['Client', 'Server', 'Facilitator', 'Blockchain'],
    uniParticipants: ['Client', 'Server'],
    tradStepLabels: [
      'GET /api',
      '402 - Payment Required',
      'Select payment method',
      'X-PAYMENT: b64 payload',
      '/verify',
      'verification',
      'Fulfill request',
      '/settle',
    ...
    eliminatedTitle: '7 STEPS ELIMINATED',
    eliminatedSub: 'No facilitator · No shared ledger · No settlement',
  },
  pt: {
    eyebrow: 'Apêndice — Protocolo',
    headLead: 'UNICITY X402 —',
    headAccent: 'A MENSAGEM É O PAGAMENTO',
    subHead: 'Pagamentos HTTP nativos com finalidade instantânea, zero taxas de gas e sem complexidade de carteira',
    tradTitle: 'X402 Blockchain Tradicional',
    tradBadge: '12 passos',
    uniTitle: 'X402 Unicity Sem Atrito',
    uniBadge: '5 passos',
    tradParticipants: ['Cliente', 'Servidor', 'Facilitador', 'Blockchain'],
    uniParticipants: ['Cliente', 'Servidor'],
    tradStepLabels: [
      'GET /api',
      '402 - Payment Required',
    ...
    eliminatedTitle: '7 PASSOS ELIMINADOS',
    eliminatedSub: 'Sem facilitador · Sem ledger compartilhado · Sem liquidação',
  },
});
          ...
          return (
            <g key={step.num}>
              ...
              <text x={x + 16} y={y + 18} fill="rgba(254,254,254,0.7)" fontSize="9"
                fontFamily="Geist Mono, monospace">{label}</text>
            </g>
          );
        }
        ...
        return (
          <g key={step.num}>
            ...
            <text x={mid} y={y + 10} textAnchor="middle" fill="rgba(254,254,254,0.65)"
              fontSize="8.5" fontFamily="Geist Mono, monospace">{label}</text>
            {/* Arrow line */}
            <line
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-red-500/15 border border-red-500/25 text-red-400 text-[10px] tracking-wider uppercase font-semibold"
                style={{ fontFamily: "'Geist Mono', monospace" }}>{T.tradBadge}</span>
            </div>
            <div className="flex-1 rounded-xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-sm p-2 sm:p-3 min-h-0 overflow-hidden">
              <Diagram participants={T.tradParticipants} steps={TRAD_STEPS} labels={T.tradStepLabels}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-orange-500/15 border border-orange-500/25 text-orange-400 text-[10px] tracking-wider uppercase font-semibold"
                style={{ fontFamily: "'Geist Mono', monospace" }}>{T.uniBadge}</span>
            </div>
            <div className="flex-1 rounded-xl border border-orange-500/20 bg-orange-500/[0.03] backdrop-blur-sm p-2 sm:p-3 min-h-0 overflow-hidden">
              <Diagram participants={T.uniParticipants} steps={UNI_STEPS} labels={T.uniStepLabels}
