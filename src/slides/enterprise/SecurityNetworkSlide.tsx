...
// Non-text data: stable ids, colors and geometry stay outside the map.
const enterprises = [
  { id: 'bank-a', color: '#3b82f6', angle: -90, dist: 210, agents: 4 },
  { id: 'insurer', color: '#ec4899', angle: -18, dist: 220, agents: 3 },
  { id: 'logistics', color: '#a855f7', angle: 54, dist: 210, agents: 3 },
  { id: 'bank-b', color: '#f59e0b', angle: 126, dist: 220, agents: 4 },
  { id: 'telco', color: '#06b6d4', angle: 198, dist: 210, agents: 3 },
];

// Non-text data: stable ids + colors outside; display title/text in T.
const keyPoints = [
  { id: 'sif', color: '#a855f7' },
  { id: 'zero-trust', color: '#f97316' },
  { id: 'p2p', color: '#f97316' },
  { id: 'isolation', color: '#3b82f6' },
  { id: 'audit', color: '#06b6d4' },
];
...
const T = tr({
  en: {
    ...
    enterpriseNames: ['BANK A', 'INSURER', 'LOGISTICS', 'BANK B', 'TELCO'],
    marketplaceLabel: 'MARKETPLACE',
    sifGateway: 'SIF GATEWAY',
    keyPoints: [
      {
  ...
  pt: {
    ...
    headLead: 'ZERO TRUST',
    headAccent: 'REDE CORPORATIVA',
    subHead: (
      <>
        Cada empresa implanta seu próprio <span className="text-orange-400">Semantic Intercept Fabric</span> como gateway — validando, auditando e aplicando políticas em cada interação de agente antes que ela chegue ao marketplace.
      </>
    ),
    enterpriseNames: ['BANCO A', 'SEGURADORA', 'LOGÍSTICA', 'BANCO B', 'TELECOM'],
    marketplaceLabel: 'MARKETPLACE',
    sifGateway: 'SIF GATEWAY',
    keyPoints: [
      {
        title: 'Semantic Intercept Fabric',
        text: 'Cada empresa implanta um gateway SIF — um proxy que aplica políticas, validando, auditando e filtrando cada interação de agente',
      },
      {
        title: 'Zero Trust por Padrão',
        text: 'Nenhum agente confia em outro. Cada mensagem é autenticada, cada transação é verificada. O gateway aplica políticas específicas da empresa sem expor o estado interno.',
      },
      {
        title: 'Marketplace P2P',
        text: 'O marketplace central permite descoberta e solicitações de transação. Agentes de empresas diferentes interagem através de seus respectivos gateways — nunca diretamente.',
      },
      {
        title: 'Isolamento Corporativo',
        text: 'Cada empresa mantém seu próprio cluster de agentes atrás de seu próprio gateway SIF. A comunicação interna entre agentes nunca deixa o perímetro.',
      },
      {
        title: 'Trilha de Auditoria',
        text: 'Toda interação entre empresas é registrada no nível do gateway — quem solicitou o quê, qual política foi aplicada e se foi aprovada ou negada.',
      },
    ],
  },
});
  ...
  return (
    <svg viewBox="0 0 600 600" className="w-full h-auto">
      ...
      <text x={CX} y={CY + 6} textAnchor="middle" fill="#f97316" fontSize="16" fontWeight="bold"
        fontFamily="Anton, sans-serif" letterSpacing="0.1em">{T.marketplaceLabel}</text>
