...
// Non-text data: stable ids + colors outside; display title/text in T.
const keyPoints = [
  { id: 'pow', color: '#6366f1' },
  { id: 'bft', color: '#6366f1' },
  { id: 'oracle', color: '#6366f1' },
  { id: 'sdk', color: '#f97316' },
  { id: 'agent', color: '#06b6d4' },
];

const T = tr({
  en: {
    eyebrow: 'Appendix — Blockchain',
    headLead: 'UNICITY',
    headAccent: 'PROTOCOL STACK',
    subHead: (
      <>
    ...
    diagConsensus: 'CONSENSUS',
    diagProofOfWork: 'PROOF OF WORK',
    diagBftFinality: 'BFT FINALITY',
    diagUniquenessOracle: 'UNIQUENESS ORACLE',
    diagOnChain: 'On-Chain',
    diagOffChain: 'Off-Chain',
    diagTokenOpsSdk: 'TOKEN OPERATIONS SDK',
    diagAgentExecLayer: 'AGENT EXECUTION LAYER',
    keyPoints: [
      {
        title: 'Proof of Work',
  ...
  pt: {
    eyebrow: 'Apêndice — Blockchain',
    headLead: 'UNICITY',
    headAccent: 'STACK DE PROTOCOLO',
    subHead: (
      <>
        Uma stack desenvolvida sob medida onde o <span className="text-orange-400">consenso permanece mínimo</span> e os agentes executam off-chain — conectados ao ledger por um fino Token Operations SDK.
      </>
    ),
    diagConsensus: 'CONSENSO',
    diagProofOfWork: 'PROVA DE TRABALHO',
    diagBftFinality: 'FINALIDADE BFT',
    diagUniquenessOracle: 'ORÁCULO DE UNICIDADE',
    diagOnChain: 'On-Chain',
    diagOffChain: 'Off-Chain',
    diagTokenOpsSdk: 'TOKEN OPERATIONS SDK',
    diagAgentExecLayer: 'CAMADA DE EXECUÇÃO DE AGENTES',
    keyPoints: [
      {
        title: 'Prova de Trabalho',
        text: 'Desenvolvido sob medida usando RandomX. Blocos a cada 2 horas, sem transações on-chain. A cadeia existe para permitir propriedades semelhantes a dinheiro em espécie dos tokens off-chain.',
      },
      {
        title: 'Finalidade BFT',
        text: 'Consenso chained Hotstuff desenvolvido sob medida. Oferece finalidade rápida de 1 segundo.',
      },
      {
        title: 'Oráculo de Unicidade',
        text: 'Desenvolvido sob medida usando provas ZK + hash-chain. Evita gasto duplo de ativos off-chain.',
      },
      {
        title: 'Token Operations SDK',
        text: 'Bibliotecas cliente desenvolvidas sob medida em JavaScript, Java e Rust. A ponte fina entre agentes off-chain e provas on-chain.',
      },
      {
        title: 'Camada de Execução de Agentes',
        text: 'Totalmente off-chain. Transporte (I2P, TCP/IP), mensageria (NOSTR), armazenamento, runtime e execução verificável — tudo peer-to-peer.',
      },
    ],
  },
});
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      <text x={cx} y={y0 + bh / 2 + 5} textAnchor="middle" fill="#c7d2fe"
        fontSize="14" fontFamily={fontTitle} letterSpacing="0.06em">{T.diagProofOfWork}</text>
      ...
      <text x={cx} y={y1 + bh / 2 + 5} textAnchor="middle" fill="#c7d2fe"
        fontSize="14" fontFamily={fontTitle} letterSpacing="0.06em">{T.diagBftFinality}</text>
      ...
      <text x={cx} y={y2 + bh / 2 + 5} textAnchor="middle" fill="#c7d2fe"
        fontSize="14" fontFamily={fontTitle} letterSpacing="0.06em">{T.diagUniquenessOracle}</text>
      ...
      <text x={cx - bw / 2 - 8} y={yDiv - 7} textAnchor="end" fill="rgba(254,254,254,0.4)"
        fontSize="9" fontFamily={font}>{T.diagOnChain}</text>
      <text x={cx - bw / 2 - 8} y={yDiv + 13} textAnchor="end" fill="rgba(254,254,254,0.4)"
        fontSize="9" fontFamily={font}>{T.diagOffChain}</text>
      ...
      <text x={cx} y={yA + 80} textAnchor="middle" fill="#06b6d4"
        fontSize="12" fontFamily={fontTitle} letterSpacing="0.08em">{T.diagAgentExecLayer}</text>
    </svg>
  );
}
